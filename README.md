...
Tracked Buff centering keeps Blizzard's pooled icon frames parented to the native viewer and positions only real, currently shown buffs against a tight addon-owned container. Hidden Blizzard layout children do not reserve centering space. The integration stores desired anchors in addon-owned weak tables and synchronizes from Blizzard refresh and active-state callbacks rather than taking ownership of the viewer's layout lifecycle.
