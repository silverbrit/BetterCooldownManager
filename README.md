...
Tracked Buff centering uses BCM-owned Retail 12.1 `CustomAuraContainerTemplate` displays instead of moving or refreshing Blizzard's pooled rows. Aura-backed spell entries compact inside a fixed-capacity centered layout; item-backed entries follow in stable BCM-owned slots that support equipment and spell-category sources such as potions and healthstones. Item slots remain present when cooldown readiness is secret, use associated AuraContainer overlays when applicable, and never anchor to the main container's secret-sized edge. While the replacement is active, the native Tracked Buff viewer's supported Edit Mode visibility and icon-scale settings are temporarily aligned with BCM so its editor preview uses the configured icon size; both native values are restored when the feature is disabled or the session ends.
